...
import { FiImage, FiDownload, FiAlertTriangle } from 'react-icons/fi';
...
const Gif = () => {
  ...
  useEffect(() => {
    if (videoRef.current) {
      const handleLoadedMetadata = () => {
        ...
        setEndTime(Math.min(duration, 5)); // Default to 5 seconds or full duration if shorter
      };
  ...
  const handleCreateGif = () => {
    ...
    const outputFilename = `${originalName}.gif`;
    ...
    const checkQueue = setInterval(() => {
      if (processingProgress === 100 && outputFiles[outputName]) {
        clearInterval(checkQueue);
        setOutputUrl(outputFiles[outputName]);
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">Create GIF from Video</h1>
      ...
      <div className="mb-8">
        <h2 className="text-lg font-semibold mb-4 flex items-center">
          <FiImage className="mr-2" /> GIF Settings
        </h2>
          ...
          <div className="mt-6">
            {isProcessing ? (
              <div>
                ...
                <p className="text-center mt-2">Creating GIF... {processingProgress}%</p>
              </div>
            ) : isComplete ? (
              <div className="text-center">
                <p className="text-green-600 font-medium mb-4">GIF created successfully!</p>
                <button
                  type="button"
                  onClick={handleDownload}
                  className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                >
                  <FiDownload className="mr-2" /> Download GIF
                </button>
              </div>
            ) : (
              <button
              ...
              >
                {isFFmpegLoaded ? 'Create GIF' : 'Loading FFmpeg...'}
              </button>
            )}
          </div>
        </div>
      ...
      <div className="bg-blue-50 border-l-4 border-blue-400 p-4">
        <h3 className="font-semibold mb-2">About GIF Creation</h3>
        <p className="text-sm">
          GIFs are perfect for sharing short, looping video clips on social media and messaging apps.
          For best results, keep your GIFs short (under 10 seconds) and consider using a lower frame rate
          and resolution to keep file sizes manageable.
        </p>
      </div>
    </div>
  );
